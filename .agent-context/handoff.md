...
- Vertical-focus report mode is implemented and verified: `oss-scan report --focus vertical` boosts explicit small-business/domain workflows and downranks generic web frameworks, component libraries, AI, infra, and developer-tool false positives.
...
Use vertical-focus reports as the default for this product direction. Shortlist and write memos for packaging-friendly small-business/domain candidates such as `akira-io/laravel-pdf-invoices`, `angelodlfrtr/go-invoice-generator`, `Anmol-Baranwal/form-builder`, `rubyforgood/casa`, `GTBitsOfGood/VolunTrack`, `ChanMeng666/automotive-repair-management-system`, `nbt4/rentalcore`, `elghaied/payload-reserve`, `open-condo-software/condo`, `microrealestate/microrealestate`, `georgewangchn/VetVoice`, `tastyigniter/TastyIgniter`, and `TDuckCloud/tduck-survey-form`. Next product feature should tighten vertical false-positive filtering and optionally add a `--min-vertical-score` report filter.
